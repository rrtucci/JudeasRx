    ...
    def run_gui(self):
        ...
        def save_but_do(btn):
            with capture_output() as cap:
                ...
                print("P_f=1-P_m=", 1-self.pmale)
                print()
                self.bounder_m.print_utility_fun('_m')
                print()
                self.bounder_f.print_utility_fun('_f')
                print("Exp. bds:--------------------------")
                self.bounder_m.print_exp_probs_bds('_m')
                print()
                self.bounder_f.print_exp_probs_bds('_f')
                print("bATE, ATE:-------------------------")
                bATE_m = self.bounder_m.get_bdoorATE()
                bATE_f = self.bounder_f.get_bdoorATE()
                bATE = bATE_m * self.pmale + bATE_f * (1 - self.pmale)
                if self.only_obs:
                    print("bATE_m=", "%.3f" % bATE_m)
                    print("bATE_f=", "%.3f" % bATE_f)
                    print("bATE=", "%.3f" % bATE)
                else:
                    ATE_m = self.bounder_m.get_ATE()
                    ATE_f = self.bounder_f.get_ATE()
                    ATE = ATE_m * self.pmale + ATE_f * (1 - self.pmale)
                    print("bATE_m, ATE_m=", "%.3f, %.3f" % (bATE_m, ATE_m))
